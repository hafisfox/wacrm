...
/**
 * Paging up to PAGE_CAP × 100 templates sequentially can outrun the
 * default serverless budget on a large WABA — the request would 504 and
 * the client could not tell that apart from a real failure. Ask for the
 * headroom explicitly.
 */
...
export async function POST() {
  try {
    ...
    while (nextUrl && pageCount < PAGE_CAP) {
      pageCount++;
      const metaRes: Response = await fetchWithTimeout(
        nextUrl,
